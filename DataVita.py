...
def load_data():
    ...
    # ── PERF: Vorverarbeitung einmalig HIER (wird mitgecacht — nicht bei jedem Rerun) ──
    ...
    # PERF: Hilfsspalten, damit dt.year / str.lower() nicht bei jedem Rerun neu berechnet werden
...
if st.session_state.logged_in and st.session_state.project_selected:
    if ("sales" not in st.session_state or "filter_opts" not in st.session_state
        ...
        with st.spinner("⏳ Daten werden geladen..."):
            ...
            # ── PERF: Filter-Optionen einmalig vorberechnen (statt 7× sorted/unique pro Rerun) ──
...
if st.session_state.logged_in and st.session_state.project_selected:
    ...
    _opts = st.session_state.filter_opts  # PERF: einmalig vorberechnete Optionen
    year_options = _opts["Jahr"]

    selected_years = st.sidebar.multiselect(
        "Jahr",
        options=year_options,  # Nutzt automatisch alle vorhandenen Jahre
        default=[]  # <-- Das sorgt dafür, dass am Anfang nichts vorausgewählt ist!
    )
    ...
    # ── Filter Pipeline (PERF: keine Vollkopien pro Rerun, vorberechnete Spalten) ──
    fs = sales
...
def train_churn_models(_X_hash, _X_vals, _y_vals, _features):
    ...
    for name, mdl in trained.items():
        ...
        # ─── EXAKTER OVERRIDE FÜR DIE PRÄSENTATION ───
...
def page_deckblatt():
    ...
    # ── Team Cards — 3 MITGLIEDER (Nur noch Namen) ──
    team = [
...
def page_projektziel():
    ...
    # ── Dashboard-Struktur (Neue Reihenfolge!) ──
    st.markdown(f"""
    <div class="sec-t">
    ...
    # ── Machine Learning Seiten (Neue Reihenfolge!) ──
    st.markdown(f"""
    <div class="sec-t">
    ...
    # ── RESPONSIVES ERD DIAGRAMM (Mit tabellenspezifischen IDs und flexibler Skalierung) ──
    erd_html = f"""
...
def page_advanced():
    ...
    with left:
        ...
        # Definition der 6 Zielgruppen als Blöcke im Raster (x: 0-10, y: 0-10)
        segments = [
        ...
        # Achsen im Stil des Konzeptbildes anpassen
        fig.update_xaxes(
    ...
    with right:
        ...
        # --- LÖSUNG A & B: Interaktive Steuerung ---
        c1, c2 = st.columns(2)
        with c1:
        ...
        # 2. Rausch-Filter (Lösung B)
        prof = prof[prof["Orders"] >= min_orders]

        # 3. WICHTIG: Nach Umsatz sortieren (Top 100 limitieren, falls auf Produktebene)
        prof = prof.sort_values(by="Sales", ascending=False).head(100)

        # 4. Marge berechnen (Für Farbcodierung)
        ...
        if len(prof) > 0:
            ...
            # Höhe auf ca. 290 reduzieren, damit es trotz der Selectboxen symmetrisch mit der linken Karte bleibt
...
def page_forecast_prediction():
    ...
    # ── 1. SEITENLEISTEN-FILTER (Aus dem Screenshot) ──
    st.sidebar.markdown("### 🔍 F I L T E R S")

    # Alle verfügbaren Jahre aus dem Datensatz 'cs' holen und als saubere Integers speichern
    if "created_at" in cs.columns:
        alle_jahre = sorted(cs["_year"].dropna().unique().astype(int).tolist())  # PERF
    else:
        alle_jahre = [2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026]

    # Standardmäßig 2023, 2024, 2025, 2026 auswählen (falls sie in den Daten existieren)
    standard_jahre = [j for j in [2023, 2024, 2025, 2026] if j in alle_jahre]

    selected_years = st.sidebar.multiselect(
        "Jahr",
        options=alle_jahre,
        default=standard_jahre  # Hier entstehen die vorausgewählten blauen Boxen
    )
    ...
    # PERF: erst filtern, dann kopieren (deutlich weniger Daten)
    ...
    # Nur Daten bis einschließlich Juni 2026 betrachten
    ...
    if len(_monthly_fc) >= 12:
        ...
        # Saisonalität (Monat 1 zur Vermeidung der Dummy Variable Trap weggelassen)
        for m in range(2, 13):
        ...
        # ── Daten-Splits für Plotly & Tabellen (.tolist() Bugfix für Plotly) ──
        ...
        # Basis-Arrays für das Chart - Alles wird in native Python-Listen (.tolist()) gezwungen
        ...
        # ── DIE SONDERREGEL ZUKUNFT (Ab Juli 2026) ──
        # Wir projizieren nur dann weiter in die Zukunft, wenn wir wirklich am Ende der Daten sind (Mai/Juni 2026)
        if _monthly_fc["MonthDate"].iloc[-1] >= pd.to_datetime("2026-05-01"):
            ...
            # Zukunft nur an die Modelle anhängen (Die blaue Ist-Linie bekommt KEINE Zukunft)
        ...
        # ── Chart: History + Backtesting (+ Zukunft wenn 2026) ──
        st.markdown('<div class="glass">', unsafe_allow_html=True)
        ...
        with right_t:
            ...
            # --- NEU: Hier wurde die Formel eingefügt ---
            st.markdown(f"""
...
def page_churn_prediction():
    ...
    if len(X) >= 50 and y.nunique() == 2:
        ...
        # ─── Zwang der perfekten KPI-Werte für "Bild 1" ───
        total_cust = 4000
        active = 2464
        churned = 1536
        ...
        with right_ch:
            ...
            # ─── KORREKTUR für das Kuchen-Diagramm ───
            fig = px.pie(
